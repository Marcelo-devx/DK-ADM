...
import { PlusCircle, Search, DownloadCloud, FileUp, FileDown, Eye, Loader2, SlidersHorizontal, X, FilterX, Package, ClipboardList, AlertTriangle } from "lucide-react";
...
export const ProductToolbar = ({
  ...
  const hasActiveFilters =
    searchTerm || brandFilter !== "all" || categoryFilter !== "all" || maxStockFilter !== "";
  ...
  return (
    <div className="mb-6">
      ...
      <div className="md:hidden">
          ...
          <Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
            ...
            <SheetContent side="bottom" className="h-[80vh] overflow-y-auto rounded-t-2xl">
              ...
              <div className="space-y-4">
                ...
                {/* Estoque baixo */}
                <div>
                  <label className="text-xs font-medium text-gray-500 mb-1 block">Estoque igual ou menor que</label>
                  <div className="relative flex items-center bg-gray-50 border border-gray-200 rounded-lg h-11 overflow-hidden">
                    <AlertTriangle className="absolute left-3 w-4 h-4 text-gray-400" />
                    <input
                      type="number"
                      min={0}
                      placeholder="Ex: 5"
                    ...
                    />
                    {maxStockFilter && (
                      <button onClick={() => onMaxStockFilterChange("")} className="absolute right-3 text-gray-400">
                        <X className="w-3.5 h-3.5" />
                      </button>
                    )}
                  </div>
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap items-center gap-1.5 mt-3">
            ...
            {maxStockFilter !== "" && (
              <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full px-2 py-0.5 text-xs">
                Estoque ≤ {maxStockFilter}
                <button onClick={() => onMaxStockFilterChange("")}><X className="w-3 h-3 ml-0.5" /></button>
              </span>
            )}
          </div>
        )}
      ...
      <div className="hidden md:flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 bg-white p-4 rounded-xl border shadow-sm">
