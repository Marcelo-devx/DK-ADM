...
export const ProductsTable = ({
  ...
  const getPriceDisplay = (product: Product, isCost: boolean = false) => {
    if (!product) return "-";
    const costsArray = Array.isArray(product.variant_costs)
      ? product.variant_costs
      : [];
    const pricesArray = Array.isArray(product.variant_prices)
      ? product.variant_prices
      : [];
    const values = isCost
      ? (costsArray.filter((v) => v !== null) as number[])
      : (pricesArray.filter((v) => v !== null) as number[]);
    const baseValue = isCost
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              onClick={() => onViewVariants({ id: product.id, name: product.name })}
          ...
          <TooltipContent>
            <p>Clique para ver as {values.length} variações</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
          ...
          {isLoading ? (
            ...
            products.map((product) => (
              ...
              >
                ...
                <TableCell className="font-mono text-[10px] font-black text-gray-500">
                  #{product.sku || product.id}
                </TableCell>
                <TableCell className="font-bold text-xs truncate max-w-[200px] text-gray-800">
                  {product.name}
                </TableCell>
                ...
                <TableCell>
                  <div className="flex flex-col gap-1">
                    <Badge
                      ...
                      className="h-5 text-[10px] w-fit font-black"
                    >
                    ...
                    {product.allocated_in_kits ? (
                      <Badge
                        variant="outline"
                        className="h-5 text-[9px] w-fit bg-amber-50 text-amber-700 border-amber-200 gap-1 font-bold"
                        title="Quantidade reservada em Kits"
                      >
                        <Lock className="w-2.5 h-2.5" /> +{" "}
                        {product.allocated_in_kits} em Kits
                      </Badge>
                    ) : null}
                  </div>
                </TableCell>
