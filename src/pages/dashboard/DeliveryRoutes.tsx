...
const DeliveryRoutesPage = () => {
  ...
  // Calcula data atual e próxima para fechar a janela de 24h na API
  const formattedDate = date ? format(date, "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd");
  const nextDate = date ? addDays(date, 1) : addDays(new Date(), 1);
  ...
  const { data: routes, isLoading, isError, error, refetch, isRefetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        // 2. Buscar Planos (Plans) do dia ESPECÍFICO
        // Usamos startsGte (>= hoje) e startsLt (< amanhã) para pegar só 24h
        setLoadingProgress("Buscando rotas do dia...");
        const rawPlans = await fetchAllPages("plans", { 
            "filter.startsGte": formattedDate, 
            "filter.startsLt": formattedNextDate, // Importante: Limita até o dia seguinte
            maxPageSize: 50 
        }, "plans");

        if (rawPlans.length === 0) return [];

        // 3. Processar Planos SEQUENCIALMENTE
        ...
        for (const plan of rawPlans) {
            ...
            // Filtro de segurança extra (Client Side)
            // Se o plano não tiver data ou a data não começar com o dia selecionado, pula.
            ...
            for (const driverId of Object.keys(stopsByDriver)) {
                ...
                detailedRoutesArray.push({
                    ...
                    originalTitle: plan.title, // Guardamos o título original para referência
