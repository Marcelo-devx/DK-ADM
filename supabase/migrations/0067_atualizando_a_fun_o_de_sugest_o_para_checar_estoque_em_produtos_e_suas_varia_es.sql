...
BEGIN
    ...
    WHERE 
        ...
        -- Verificação Robusta de Estoque para o Produto A
        AND (
            pr1.stock_quantity > 0 
            OR 
            EXISTS (SELECT 1 FROM public.product_variants pv WHERE pv.product_id = pr1.id AND pv.stock_quantity > 0)
        )
        -- Verificação Robusta de Estoque para o Produto B
        AND (
            pr2.stock_quantity > 0 
            OR 
            EXISTS (SELECT 1 FROM public.product_variants pv WHERE pv.product_id = pr2.id AND pv.stock_quantity > 0)
        )
        -- Visibilidade
