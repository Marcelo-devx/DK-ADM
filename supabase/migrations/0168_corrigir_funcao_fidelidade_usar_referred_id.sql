-- Corrigir função de processamento de fidelidade para usar a coluna correta (referred_id em vez de referee_id)
-- Problema: A função estava tentando acessar a coluna referee_id que não existe na tabela referrals
-- A coluna correta é referred_id, e não existe a coluna completed_at, devemos usar a coluna status
...
BEGIN
    ...
    IF NEW.status IN ('Pago', 'Finalizada') AND (OLD.status NOT IN ('Pago', 'Finalizada')) THEN
        ...
        IF v_referrer_id IS NOT NULL THEN
            -- Verifica se já processou essa indicação (status = 'pending')
