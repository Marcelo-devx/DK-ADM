...
serve(async (req) => {
  ...
  if (term) {
    ...
    // Busca direta na tabela profiles (email, cpf/cnpj, telefone, nome já cobrem os casos —
    // não é necessário percorrer auth.users, pois todo profile já tem email preenchido)
    const selectFields =
    ...
    if (countRes.error) console.error("[admin-list-users] Profile count error:", countRes.error);
    if (dataRes.error) console.error("[admin-list-users] Profile fields query error:", dataRes.error);

    console.log(`[admin-list-users] profiles query result: count=${countRes.count}, data=${dataRes.data?.length}`);

    countResult = { count: countRes.count ?? 0, error: countRes.error };
    dataResult = { data: dataRes.data ?? [], error: dataRes.error };
