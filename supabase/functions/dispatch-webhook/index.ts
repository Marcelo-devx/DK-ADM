...
serve(async (req) => {
  ...
  if (req.method === 'OPTIONS') {
    ...
    const corsHeaders = buildSecureCorsHeaders(origin);
    return new Response('ok', { headers: corsHeaders });
  }

  try {
    // 🔒 SECURITY: Check origin before processing
    const origin = req.headers.get('origin');
    const { isAllowed } = checkOrigin(origin, req.method);
    
    if (!isAllowed) {
      console.warn('[dispatch-webhook] Origin not allowed:', origin);
    ...
    if (!isServiceRole && !currentUser) {
      return new Response(JSON.stringify({ error: 'Unauthorized' }), { 
        status: 401, 
        headers: buildSecureCorsHeaders(origin)
      });
    }

    const { event_type, payload } = await req.json();

    if (!event_type) return new Response("Missing event_type", { status: 400, headers: buildSecureCorsHeaders(origin) });
    ...
    if (!orderId && ['order_created', 'order_paid', 'order_shipped', 'order_delivered'].includes(event_type)) {
      return new Response("Missing order ID", { status: 400, headers: buildSecureCorsHeaders(origin) });
    }
    ...
    if (event_type === 'order_created') {
      ...
      if (duplicate) {
        return new Response(JSON.stringify({ success: true, skipped: true, message: "Duplicate skipped" }), { 
    ...
    if (['order_created', 'order_paid', 'order_shipped', 'order_delivered'].includes(event_type)) {
      ...
      if (!isServiceRole && currentUser) {
        if (order.user_id !== currentUser.id) {
          // If user doesn't own order, check if they are an admin
